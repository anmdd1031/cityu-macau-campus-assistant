...
本指南面向中国大陆用户。优先把项目安装为可重复使用的 Skill；不支持 Skills 的聊天工具再使用单文件上传。

## 先选一种方案

| 方案 | 适合谁 | 是否上传云端 | 预计时间 |
|---|---|---:|---:|
| A. 安装为 Agent Skill | Codex 或支持 Skills 的客户端 | 否 | 2-5 分钟 |
| B. 国内 AI 直接上传 | 不支持 Skills 的聊天工具 | 是 | 2 分钟 |
| C. Open WebUI / Dify 知识库 | 长期使用、团队共享 | 取决于部署方式 | 20-60 分钟 |
| D. Ollama + Qwen 本地运行 | 注重隐私、有较好电脑配置 | 否 | 30-90 分钟 |
| E. MCP 客户端 | 不支持 Skills 但支持 MCP 的客户端 | 本地读取 | 10-30 分钟 |

不确定时直接选择 **方案 A**。

## 方案 A：安装为可重复使用的 Skill
...
Codex 会把 Skill 安装到本地 Skills 目录。安装完成后重启 Codex，然后可以在任意新对话中调用：

```text
使用 $cityu-macau-campus-assistant 查询内地新生注册流程。
```

Skill 会根据问题按需读取 `references/freshman.md` 或 `references/fds.md`，因此不需要每次重新上传文件。

### 其他兼容 Agent Skills 的客户端

1. 下载并解压本仓库。
2. 找到 `skills/cityu-macau-campus-assistant`。
3. 将整个文件夹复制到客户端指定的 Skills 目录。
4. 重启客户端。
5. 检查 Skill 列表中是否出现“澳门城市大学校园助手”。

不要只复制 `SKILL.md`，否则 Agent 无法读取配套参考资料。
...
本仓库包含一个标准 Agent Skill，但**不是独立 MCP Server**。只有客户端不支持 Skills、但支持 MCP 时，才需要用 Filesystem MCP 读取本地仓库。
...
不要直接编辑 `dist` 文件，应修改 `skills/cityu-macau-campus-assistant/SKILL.md` 或该 Skill 的 `references/` 后重新生成。
