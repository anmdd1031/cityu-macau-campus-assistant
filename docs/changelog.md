...
- 更正本科转入数据科学学院的最低 GPA：智能科技与服务学士（BITS）为 3.0，计算机科学学士（BCS）为 3.5；旧版 BCS 3.7 数据有误。
...
- FDS：维护脚本改为中文官网优先、默认单线程；以 `--workers 1 --retries 0 --timeout 8 --delay 0.8 --check` 完成 58 个教师页在线检查，结果无漂移、无人工复核项。
